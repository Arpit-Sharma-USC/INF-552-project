...
def generate_model(is_self_training=False):
    ...
    vectors_normalized = pr.normalize(vectors_scaled, norm='l1')
    ...
    if classifier_type == cons.CLASSIFIER_SVM:
        ...
        # model = LogisticRegressionCV(Cs=Cs,class_weight=class_weights,refit=True,cv=5)
...
def predict(tweet, is_self_training):
    ...
    # z=np.array(z)
    # z=z.reshape(-1,1)
    # z_scaled = ds.SCALAR.transform(z.ravel())
    z_scaled=z
    # z_scaled = MaxAbsScaler(z)
